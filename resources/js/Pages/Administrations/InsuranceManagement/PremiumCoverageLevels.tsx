...
const PremiumCoverageLevels = ({insurance_type_id, coverage_levels, year_id, year}:any) => {
        ...
        <MainLayout title="Insurance Setups | Premium Rates">
            <Breadcrumbs title={
                <div className='flex flex-row gap-1 items-center'>
                    ...
                    <h4>{year} Premium Coverage Levels</h4>
                </div>
            } />

            <TopHeaderSection title={`${year} Premium Coverage Levels`} hideAddBtn={true} />

            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
                {coverage_levels.map((item:any, index:number) => (
                    <Link href={`/administrations/insurance-types/${insurance_type_id}/years/${year_id}/premium-coverage-levels/${item.id}`} className="flex flex-col items-center justify-center border p-3 gap-2 rounded" key={index}>
...
export default PremiumCoverageLevels;